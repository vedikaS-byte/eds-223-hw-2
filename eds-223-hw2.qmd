...
#Keep all rows from ej_screen even if  don’t intersect any inequal polygon , If a census block doesn’t fall inside a HOLC area, its new grade column will be NA
# Now we can drop geometries for summary stats
# We can create a table to check for the count of redlined neighborhoods in census blocks
table(census_holc_df$grade, useNA = "ifany")
str(census_holc_df)

ej_la <- st_filter(ej_screen, inequal, .predicate = st_intersects) # get data just for LA
census_holc <- st_join(ej_la, inequal, left = TRUE)
...
census_holc_df %>%
  ...
  group_by(grade) %>%
  summarise(
    mean_pm25_lowinc = mean(pm25_lowinc_pctl, na.rm = TRUE),
    mean_lifeexp = mean(lifeexp_pctl, na.rm = TRUE)
  )
str(census_holc_df)
# - percentile for low life expectancy 
...
str(birds)
# Next, perform a spatial join between birds and holc
birds_holc <- st_join(inequal, birds, left = T)
birds_holc_df <- st_drop_geometry(birds_holc)
# Then, filter for 2021-2023 only
colnames(birds_holc_df)

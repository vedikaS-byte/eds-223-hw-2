...
```{r}
# Check and match CRS for all

# Compare ej_screen and birds
if (st_crs(ej_screen) == st_crs(birds)) {
  message("CRS match: ej_screen and birds.")
} else {
  warning("CRS mismatch: Transform birds to match ej_screen.")
  birds <- st_transform(birds, st_crs(ej_screen))
}
```

```{r}
# Compare ej_screen and inequal
if (st_crs(ej_screen) == st_crs(inequal)) {
  message("CRS match: ej_screen and inequal.")
} else {
  warning("CRS mismatch: Transform inequal to match ej_screen.")
  inequal <- st_transform(inequal, st_crs(ej_screen))
}
```

```{r}
# Compare birds and inequal, include stop() if still no match
if (st_crs(birds) == st_crs(inequal)) {
  message("Final check: CRS match between birds and inequal.")
} else {
  stop("CRS mismatch between birds and inequal: 
       Double check transformations for matching CRS 
...
#| warning: false
#| message: false
#Create a custom color palette that matches the actual HOLC grade
...
# Establish basemap with tm_tiles using view from Leaflet
  ...
  tm_polygons(
    ...
    palette = grade_colors, # Utilize color palette
    na.color = "grey",
...
***Map 1. Finalized map of census blocks within or located outside a
HOLC graded redlined area. Areas graded “A” (green) were considered
“safe,” while those graded “D” (red) were considered “unsafe.”***
...
```{r}
# Check that levels in socio_economic column have been successfully renamed 
if ("socio_economic" %in% names(holc_summary)) {
  holc_summary$socio_economic <- factor( #Convert to factor
    holc_summary$socio_economic,
    levels = c("mean_low_income", "mean_low_life_exp", "mean_pm25"),
    labels = c("Average Low Income", 
               "Average Low Life Expectancy", 
               "Average Particulate Matter Concentration (PM 2.5)")
  )
  # Pass message if levels successfully renamed
  message("'socio_economic' factor labels successfully applied.")
  
} else { 
  # Stop running cell if socio_economic not successfully created 
  stop("Error: Column 'socio_economic' not found in 'holc_summary'.")
}
...
```{r}
# Create a bar graph to plot mean value for each variable by HOLC grade
...
# Store graph in figs
#ggsave("figs/Mean_EJScreen_Indicators_by_HOLCGrade.png", plot = grade_avg_socio)
```

![](figs/Mean_EJScreen_Indicators_by_HOLCGrade.png)

***Figure 1. Finalized graph of mean percentile of environmental justice
...
***Figure 2. Finalized heatmap of mean percentile of environmental
...
```{r}
# Double check that birds and inequal are in 
# the same CRS once more 

# Compare birds and inequal, include stop() if still no match
if (st_crs(birds) == st_crs(inequal)) {
  message("Final check: CRS match between birds and inequal.")
} else { # Stop running cell if there is a mismatch
  stop("CRS mismatch between birds and inequal: 
       Double check transformations for matching CRS between 
       spatial objects!")
}
...
bird_summary %>% 
  ...
  labs(
    title = "Percent of Bird Observations in Redlined Neighborhoods 
    Within HOLC Grades (2021–2023)",
    x = "HOLC Grade",
    y = "Percent of Bird Observations" ) +
  theme_bw() 
```

![](figs/Percent_of_Bird_Observations_in_Redlined_Neighborhoods_Within_HOLC_Grades.png)

***Figure 3. Finalized bar graph of percent bird observations made in
redlined neighborhoods per HOLC grade.***

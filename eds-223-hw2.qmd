...
holc_summary <- census_holc_df %>%
  ...
  summarise( # Find the mean of each socioeconimic variable
...
# Store graph in figs
ggsave("figs/Mean_EJScreen_Indicators_by_HOLC_Grade.png", grade_avg_socio)
```
...
# Store graph in figs
ggsave("figs/Heatmap_Mean_EJScreen_Indicators_by_HOLC_Grade.png", heatmap)
```
...
Create a visualization that shows: - The percentage of bird observations
within each HOLC grade
...
# Creat a pallete for each grade (% of bird observations per HOLC grade)
